---
title: "Organizing a Flow Catalog"
---

<Warning>
  This page is outdated. It does not reflect the current state of the Flow web application and the [authorization model](/reference/authentication/) used to share entities in Flow catalogs. Updates are coming soon.
</Warning>

It's not necessary to store the entire catalog spec in one YAML file, and Flow provides the flexibility to reference other files, which can be managed independently. You can leverage this capability when you [run Flow from the command line](/concepts/flowctl/). You may want to do so if:
...
#### Example: Organizing collections[​](#example-organizing-collections "Direct link to Example: Organizing collections")
...
```
acme/customers/customerInfoacme/products/info/manufacturersacme/products/info/skusacme/products/inventoryacme/sales/pendingacme/sales/complete
```

```
acme├── flow.yaml├── customers│   ├── flow.ts│   ├── flow.yaml│   └── schemas.yaml├── products│   ├── flow.yaml│   ├── info│   │   ├── flow.ts│   │   ├── flow.yaml│   │   └── schemas.yaml│   └── inventory│       ├── flow.ts│       ├── flow.yaml│       └── schemas.yamlschemas.yaml└── sales    ├── flow.ts    ├── flow.yaml    └── schemas.yaml
```
...
```
import:  - customers/flow.yaml  - products/flow.yaml  - sales/flow.yaml
```
...
#### Example: Separate environments[​](#example-separate-environments "Direct link to Example: Separate environments")
...
```
acme├── dev.flow.yaml├── prod.flow.yaml... the remainder is the same as above
```
...
```
  import:  - customers/flow.yaml  - products/flow.yaml  - sales/flow.yaml  ourMaterializationEndpoint:    # dev.flow.yaml    sqlite:      path: dev-materializations.db
```
...
```
import:  - customers/flow.yaml  - products/flow.yaml  - sales/flow.yamlendpoints:    snowflake:      account: acme_production      role: admin      schema: snowflake.com/acmeProd      user: importantAdmin      password: abc123      warehouse: acme_production
```
...
#### Example: Cross-team collaboration[​](#example-cross-team-collaboration "Direct link to Example: Cross-team collaboration")
...
```
collection:    userProfile:        schema:            -"/userProfile/schema"        key:            [/id]
```
...
```
import:    -http://www.acme.com/teamUser#userProfile.flow.yaml    -webStuff.flow.yaml
```
...
```
collection:    webStuff:        schema:            -http://acme.com/teamUser#userProfile/#schema        key:            [/id]
```

### Global namespace[​](#global-namespace "Direct link to Global namespace")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/features/organizing-catalogs.md)
