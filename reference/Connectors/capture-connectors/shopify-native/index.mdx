...
### Access Token Permissions[​](#access-token-permissions "Direct link to Access Token Permissions")
...
### Bulk Query Operation Limitations[​](#bulk-query-operation-limitations "Direct link to Bulk Query Operation Limitations")
...
You configure connectors either in the Flow web app, or by directly editing the catalog specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the Shopify source connector.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-shopify-native:dev        config:            advanced:                window_size: P30D            credentials:                credentials_title: Private App Credentials                access_token: <secret>            start_date: "2025-01-16T12:00:00Z"            store: <store ID>    bindings:      - resource:          name: products        target: ${PREFIX}/products
```

## Limitations with Custom App Access Tokens[​](#limitations-with-custom-app-access-tokens "Direct link to Limitations with Custom App Access Tokens")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/shopify-native.md)
