---
title: "NetSuite SuiteTalk REST (Deprecated)"
---

<Warning>
  This connector is deprecated. For new pipelines, we recommend you use our actively maintained [NetSuite connector](/reference/Connectors/capture-connectors/netsuite-suiteanalytics/) instead.
</Warning>
...
## SuiteAnalytics vs SuiteQL via REST API[​](#suiteanalytics-vs-suiteql-via-rest-api "Direct link to SuiteAnalytics vs SuiteQL via REST API")

These two different connection modes have some key differences:

### [SuiteAnalytics Connect](/reference/Connectors/capture-connectors/netsuite-suiteanalytics/)[​](#suiteanalytics-connect "Direct link to suiteanalytics-connect")
...
### SuiteQL via REST API[​](#suiteql-via-rest-api "Direct link to SuiteQL via REST API")
...
* A custom role with access to objects you want to capture *or* a purchased SuiteAnalytics Module. See [setup](#general-setup).
...
You configure connectors either in the Flow web app, or by directly editing the catalog specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the NetSuite source connector.
...
| ------------------ | --------------- | -------------------------------------------------------------------------------------- | ------ | ---------------- |
| `/account_id`      | Realm           | Netsuite realm e.g. 2344535, as for `production` or 2344535\_SB1, as for the `sandbox` | string | Required         |
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-netsuite:dev        config:          account_id: <your account id>          consumer_key: <key>          consumer_secret: <secret>          token_key: <key>          token_secret: <secret>          start_date: "2023-11-01T00:00:00Z"    bindings:      - resource:          stream: Transaction        target: ${PREFIX}/Transaction      {...}
```

[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/netsuite-suitetalk.md)
