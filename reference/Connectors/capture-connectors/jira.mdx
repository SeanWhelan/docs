...
description: "This connector captures data from Jira into Flow collections."
---
...
<Warning>
  This connector is deprecated. See the new [Jira connector](/reference/Connectors/capture-connectors/jira-native/) for the latest integration.
</Warning>
...
You configure connectors either in the Flow web app, or by directly editing the catalog specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the Jira source connector.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-jira:dev        config:          api_token: <token>          domain: <domain>          email: <email>    bindings:      - resource:          stream: issues          syncMode: incremental        target: ${PREFIX}/issues      {...}
```

[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/jira.md)
