...
description: "Neon's logical replication feature allows you to replicate data from your Neon Postgres database to external destinations."
---
...
### 1. Enable Logical Replication in Neon[​](#1-enable-logical-replication-in-neon "Direct link to 1. Enable Logical Replication in Neon")
...
### 2. Create a Postgres Role for Replication[​](#2-create-a-postgres-role-for-replication "Direct link to 2. Create a Postgres Role for Replication")
...
```
curl 'https://console.neon.tech/api/v2/projects/hidden-cell-763301/branches/br-blue-tooth-671580/roles' \  -H 'Accept: application/json' \  -H "Authorization: Bearer $NEON_API_KEY" \  -H 'Content-Type: application/json' \  -d '{  "role": {    "name": "cdc_role"  }}' | jq
```

### 3. Grant Schema Access to Your Postgres Role[​](#3-grant-schema-access-to-your-postgres-role "Direct link to 3. Grant Schema Access to Your Postgres Role")
...
### 4. Create the watermarks table, grant privileges, and create publication:[​](#4-create-the-watermarks-table-grant-privileges-and-create-publication "Direct link to 4. Create the watermarks table, grant privileges, and create publication:")
...
## Allow Inbound Traffic[​](#allow-inbound-traffic "Direct link to Allow Inbound Traffic")

If you are using Neon's **IP Allow** feature to limit the IP addresses that can connect to Neon, you will need to allow inbound traffic from Estuary's IP addresses. Refer to the [Estuary documentation](/reference/allow-ip-addresses/) for the list of IPs that need to be allowlisted for the Estuary region of your account. For information about configuring allowed IPs in Neon, see [Configure IP Allow](https://neon.tech/docs/introduction/ip-allow).

## Create a Postgres Source Connector in Estuary[​](#create-a-postgres-source-connector-in-estuary-flow "Direct link to Create a Postgres Source Connector in Estuary")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/PostgreSQL/neon-postgres.md)
