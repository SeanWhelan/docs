...
It is recommended that you create a dedicated Postgres role for replicating data. The role must have the `REPLICATION` privilege. The default Postgres role created with your Neon project and roles created using the Neon Console, CLI, or API are granted membership in the neon\_superuser role, which has the required `REPLICATION` privilege.
...
If you are using Neon's **IP Allow** feature to limit the IP addresses that can connect to Neon, you will need to allow inbound traffic from Estuary Flow's IP addresses. Refer to the [Estuary Flow documentation](/reference/allow-ip-addresses/) for the list of IPs that need to be allowlisted for the Estuary Flow region of your account. For information about configuring allowed IPs in Neon, see [Configure IP Allow](https://neon.tech/docs/introduction/ip-allow).

## Create a Postgres Source Connector in Estuary Flow[​](#create-a-postgres-source-connector-in-estuary-flow "Direct link to Create a Postgres Source Connector in Estuary Flow")
