---
title: "Survey Monkey"
description: "This connector captures data from SurveyMonkey surveys into Flow collections via the SurveyMonkey API."
---
...
#### OAuth authentication in the web app[​](#oauth-authentication-in-the-web-app "Direct link to OAuth authentication in the web app")

You'll need the username and password of a SurveyMonkey user that is part of the [team](https://help.surveymonkey.com/en/billing/teams/) for which the private app was created.

#### Manual authentication with flowctl[​](#manual-authentication-with-flowctl "Direct link to Manual authentication with flowctl")
...
The SurveyMonkey API imposes [call limits](https://developer.surveymonkey.com/api/v3/#request-and-response-limits) of 500 per day and 120 per minute.

This connector uses caching to avoid exceeding these limits.

## Configuration[​](#configuration "Direct link to Configuration")

You configure connectors either in the Flow web app, or by directly editing the catalog specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the SurveyMonkey source connector.

### Properties[​](#properties "Direct link to Properties")

#### Endpoint[​](#endpoint "Direct link to Endpoint")

The properties in the table below reflect the manual authentication method. If you're working in the Flow web app, you'll use [OAuth2](#oauth-authentication-in-the-web-app), so many of these properties aren't required.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-surveymonkey:dev        config:          credentials:            access_token: {secret}            client_id: XXXXXXXXXXXXXXXX            client_secret: {secret}          start_date: 2021-01-25T00:00:00Z    bindings:      - resource:          stream: surveys          syncMode: incremental        target: ${PREFIX}/surveys      {...}
```

[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/survey-monkey.md)
