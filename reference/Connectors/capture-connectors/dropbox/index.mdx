---
title: "Dropbox"
description: "This connector captures data from a Dropbox account into a Flow collection."
---
...
By default, Flow automatically detects the file type and compression method. If necessary, you can specify the correct file type, compression, and other properties (CSV only) using the optional [parser configuration](#advanced-parsing-dropbox-files).
...
You configure the Dropbox source connector in the Flow web app. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the Dropbox source connector.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-http-file:dev        config:          path: "/my_folder"          matchKeys: ".*\.json"    bindings:      - resource:          stream: /my_folder        target: ${PREFIX}/${COLLECTION_NAME}
```

### Advanced: Parsing Dropbox files[​](#advanced-parsing-dropbox-files "Direct link to Advanced: Parsing Dropbox files")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/dropbox.md)
