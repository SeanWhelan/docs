---
title: "HTTP Ingest (Webhook)"
description: "The HTTP Ingest connector allows you to capture data from  HTTP requests. A common use case is to capture webhook deliveries, turning them into a Flow collection."
---

If you need to capture a dataset hosted at an HTTP endpoint, see the [HTTP File](/reference/Connectors/capture-connectors/http-file/) connector.
...
This connector is different from most other capture connectors in that it's not designed to pull data from a specific system or endpoint. It requires no endpoint-specific configuration, and can accept any and all valid JSON objects from any source.
...
![](/images/reference/Connectors/capture-connectors/http-ingest/assets/images/webhook-url-96c1d867200df9f24092c6d9d51319e5.png)
...
```
{  "_meta": {    "webhookId": "...",    "query": {      "testKey": "testValue"    },    ...  }  ...}
```

### Send sample data to Flow[​](#send-sample-data-to-flow "Direct link to Send sample data to Flow")
...
1. After publishing the capture, click the endpoint link from the confirmation dialog to open the Swagger UI page for your capture.
   ...
   ![](/images/reference/Connectors/capture-connectors/http-ingest/assets/images/webhook-swagger-docs-79be2bcd4a2dc7f2e82d0dce333aeee5.png)
...
3. After sending data, go to the Collections page of the Flow web app and find the collection associated with your capture. Click **Details** to view the data preview.
...
```
{  "_meta": {    "webhookId": "...",    "pathParams": {      "fooId": "123"    },    "reqPath": "/foo/{fooId}",    ...  }  ...}
```
...
### Custom collection IDs[​](#custom-collection-ids "Direct link to Custom collection IDs")
...
### Webhook signature verification[​](#webhook-signature-verification "Direct link to Webhook signature verification")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/http-ingest.md)
