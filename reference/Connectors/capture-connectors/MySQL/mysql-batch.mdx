---
title: "MySQL Batch Query Connector"
description: "This connector captures data from MySQL databases into Flow collections by periodically executing queries and translating the results into JSON documents."
---

## When to use this connector[​](#when-to-use-this-connector "Direct link to When to use this connector")

We recommend using our [MySQL CDC Connector](http://go.estuary.dev/source-mysql) instead when possible. CDC provides lower latency data capture, delete and update events, and typically has a smaller impact on the source database.
...
```
-- Grant SELECT on a specific databaseGRANT SELECT ON my_database.* TO flow_capture;-- Or grant SELECT on all databasesGRANT SELECT ON *.* TO flow_capture;
```
...
Overriding the query template is best left to power-users or done at the direction of Estuary support. Consult the [connector source](https://github.com/estuary/connectors/blob/main/source-mysql-batch/main.go) for the current text of the default template.

[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/MySQL/mysql-batch.md)
