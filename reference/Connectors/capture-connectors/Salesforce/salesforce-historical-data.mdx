---
title: "Salesforce — Historical data (deprecated)"
description: "This connector captures data from Salesforce objects into Flow collections. It uses batch processing and is ideal for syncing your historical Salesforce data."
---

[A separate connector is available for real-time Salesforce data capture](/reference/Connectors/capture-connectors/Salesforce/salesforce-real-time/). For help using both connectors in parallel, [contact your Estuary account manager](mailto:info@estuary.dev).

This connector is available for use in the Flow web application. For local development or open-source workflows, [`ghcr.io/estuary/source-salesforce:dev`](https://ghcr.io/estuary/source-salesforce:dev) provides the latest connector image. You can also follow the link in your browser to see past image versions.

<Warning>
  This connector is deprecated. Consider using the new [native Salesforce connector](/reference/Connectors/capture-connectors/Salesforce/salesforce-native/) instead.
</Warning>
...
* During [capture creation in the web application](/guides/create-dataflow/#create-a-capture), remove the bindings for objects you don't want to capture.

## Prerequisites[​](#prerequisites "Direct link to Prerequisites")

### Using OAuth2 to authenticate with Salesforce in the Flow web app[​](#using-oauth2-to-authenticate-with-salesforce-in-the-flow-web-app "Direct link to Using OAuth2 to authenticate with Salesforce in the Flow web app")
...
### Configuring the connector specification manually[​](#configuring-the-connector-specification-manually "Direct link to Configuring the connector specification manually")
...
#### Create a read-only Salesforce user[​](#create-a-read-only-salesforce-user "Direct link to Create a read-only Salesforce user")
...
3. [Create a new user](https://help.salesforce.com/s/articleView?id=sf.adding_new_users.htm\&type=5), applying the profile you just created. You'll use this user's email address and password to authenticate Salesforce in Flow.

#### Create a developer application and generate authorization tokens[​](#create-a-developer-application-and-generate-authorization-tokens "Direct link to Create a developer application and generate authorization tokens")
...
You configure connectors either in the Flow web app, or by directly editing the Flow specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the batch Salesforce source connector.

### Formula Fields[​](#formula-fields "Direct link to Formula Fields")
...
### Slowly Changing Dimensions Type 2[​](#slowly-changing-dimensions-type-2 "Direct link to Slowly Changing Dimensions Type 2")
...
### Merging Real Time and Batch Data[​](#merging-real-time-and-batch-data "Direct link to Merging Real Time and Batch Data")
...
```
---type: objectadditionalProperties: trueproperties:  Id:    type:      - string  ...required:  - Id# Your collection must include this line. If missing, please add belowreduce:  strategy: merge
```
...
The properties in the table below reflect the manual authentication method. If you're working in the Flow web app, you'll use [OAuth2](#using-oauth2-to-authenticate-with-salesforce-in-the-flow-web-app), so you won't need the `/credentials` values listed here.
