---
title: "Overview"
description: "This is a change data capture (CDC) connector that captures change events from a MariaDB database via the . It's derived from the , so the same configuration applies, but the setup steps look somewhat different."
---
...
- The [`binlog_format`](https://mariadb.com/kb/en/binary-log-formats/) system variable must be set to `ROW`.
...
### Self Hosted MariaDB[​](#self-hosted-mariadb "Direct link to Self Hosted MariaDB")
...
```
CREATE USER IF NOT EXISTS flow_capture IDENTIFIED BY 'secret'GRANT REPLICATION CLIENT, REPLICATION SLAVE ON *.* TO 'flow_capture';GRANT SELECT ON *.* TO 'flow_capture';
```
...
### Azure Database for MariaDB[​](#azure-database-for-mariadb "Direct link to Azure Database for MariaDB")
...
1. Allow connections to the database from the Estuary IP address.
   ...
   1. Create a new [firewall rule](https://learn.microsoft.com/en-us/azure/mariadb/howto-manage-firewall-portal) that grants access to the [Estuary IP addresses](/reference/allow-ip-addresses/).
...
2. Set the `binlog_expire_logs_seconds` [server perameter](https://learn.microsoft.com/en-us/azure/mariadb/howto-server-parameters#configure-server-parameters) to `2592000`.
...
## Setting the MariaDB time zone[​](#setting-the-mariadb-time-zone "Direct link to Setting the MariaDB time zone")

MariaDB's [`time_zone` server system variable](https://mariadb.com/kb/en/server-system-variables/#system_time_zone) is set to `SYSTEM` by default. Flow is not able to detect your time zone when it's set this way, so you must explicitly set the variable for your database.
...
You configure connectors either in the Flow web app, or by directly editing the catalog specification file. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the MariaDB source connector.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-mariadb:dev        config:          address: "127.0.0.1:3306"          user: "flow_capture"          password: "secret"    bindings:      - resource:          namespace: ${TABLE_NAMESPACE}          stream: ${TABLE_NAME}        target: ${PREFIX}/${COLLECTION_NAME}
```
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/MariaDB/MariaDB.md)
