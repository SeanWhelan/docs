---
title: "Navan"
description: "This connector captures data from Navan into Flow collections."
---
...
The Navan API requires that an OAuth application is set up in the Admin dashboard. The `client_id` and `client_secret` are then used to authenticate the connector to your OAuth app to request data. See [https://app.navan.com/app/helpcenter/articles/travel/admin/other-integrations/booking-data-integration](https://app.navan.com/app/helpcenter/articles/travel/admin/other-integrations/booking-data-integration) for more details.

## Configuration[​](#configuration "Direct link to Configuration")

You configure connectors either in the Flow web app, or by directly editing the catalog specification files. See [connectors](/concepts/connectors/#using-connectors) to learn more about using connectors. The values and specification sample below provide configuration details specific to the Navan source connector.
...
```
captures:  ${PREFIX}/${CAPTURE_NAME}:    endpoint:      connector:        image: ghcr.io/estuary/source-navan:dev        config:            credentials:              credentials_title: OAuth Credentials              client_id: secret_client_id_value              client_secret: secret_client_secret_value    bindings:      - resource:          name: bookings        target: ${PREFIX}/bookings
```

[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/reference/Connectors/capture-connectors/navan.md)
