...
description: "When collection specifications and schemas change, you must make corresponding changes in other parts of your Data Flow to avoid errors. In this guide, you'll learn how to respond to different types of collection changes."
---

Manual methods (using flowctl) as well as features available in the Flow web app are covered here. For an in-depth overview of the automatic schema evolution feature in the web app and how it works, see [this article](/concepts/advanced/evolutions/).

## Introduction[​](#introduction "Direct link to Introduction")

Flow [collections](/concepts/collections/) serve not only as your real-time data storage, but also as a contract between tasks that produce and consume their data. **Captures** are producers, **materializations** are consumers, and **derivations** can act as either.
...
## Schema evolution scenarios[​](#schema-evolution-scenarios "Direct link to Schema evolution scenarios")
...
<Info>
  There are a variety of reasons why these properties may change, and also different mechanisms for detecting changes in source data. In general, it doesn't matter why the collection spec has changed, only *what* has changed. However, [AutoDiscovers](/concepts/captures/#automatically-update-captures) are able to handle some of these scenarios automatically. Where applicable, AutoDiscover behavior will be called out under each section.
</Info>

### Re-creating a collection[​](#re-creating-a-collection "Direct link to Re-creating a collection")
...
![](/images/guides/schema-evolution/assets/images/evolution-re-create-ui-d8ae2ab634980751f8c87a0483f96d39.png)
...
```
captures:  acmeCo/inventory/source-postgres:    endpoint:      connector:        image: ghcr.io/estuary/source-postgres:v1        config: encrypted-pg-config.sops.yaml    bindings:      - resource:          namespace: public          stream: anvils          mode: Normal        backfill: 1        target: acmeCo/inventory/anvils_v2collections:  acmeCo/inventory/anvils_v2:    key: [/sku]    schema:      type: object      properties:        sku: { type: string }        warehouse_id: { type: string }        quantity: { type: integer }      required: [sku, warehouse_id, quantity]materializations:  acmeCo/data-warehouse/materialize-snowflake:    endpoint:      connector:        image: ghcr.io/estuary/materialize-snowflake:v1        config: encrypted-snowflake-config.sops.yaml    bindings:      - source: acmeCo/inventory/anvils_v2        backfill: 1        resource:          table: anvils          schema: inventory
```
...
If you enabled the option to [**Automatically keep schemas up to date** (`autoDiscover`)](/concepts/captures/#automatically-update-captures) and selected **Breaking change re-versions collections** (`evolveIncompatibleCollections`) for the capture, this evolution would be performed automatically.

### A new field is added[​](#a-new-field-is-added "Direct link to A new field is added")

*Scenario: this is one way in which the schema can change.*

When a new field appears in the collection schema, it *may* automatically be added to any materializations that use `recommended` fields. Recommended fields are enabled by default in each binding. See [the materialization docs](/concepts/materialization/#projected-fields) for more info about how to enable or disable `recommended` fields.
...
### A field's data type has changed[​](#a-fields-data-type-has-changed "Direct link to A field's data type has changed")
...
### A field was removed[​](#a-field-was-removed "Direct link to A field was removed")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/guides/schema-evolution.md)
