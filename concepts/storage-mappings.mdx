...
description: "Flow stores the documents that comprise your collections in a cloud storage bucket. Your  tells Flow which bucket to use."
---
...
You can set up a [bucket lifecycle policy](#bucket-lifecycle-policies) to manage data retention in your storage mapping; for example, to remove data after six months.

## Recovery logs[​](#recovery-logs "Direct link to Recovery logs")
...
The recovery logs of a task are always prefixed by `recovery/`, so a task named `acmeCo/produce-TNT` would have a recovery log called `recovery/acmeCo/produce-TNT`
...
## Bucket lifecycle policies[​](#bucket-lifecycle-policies "Direct link to Bucket lifecycle policies")
...
Bucket lifecycle policies should **only** be applied to the `collection-data/` subfolder in a bucket. Deleting data in the [`recovery/`](#recovery-logs) folder can cause tasks to fail.
...
### Add a lifecycle policy in AWS[​](#add-a-lifecycle-policy-in-aws "Direct link to Add a lifecycle policy in AWS")
...
### Add a lifecycle policy in GCP[​](#add-a-lifecycle-policy-in-gcp "Direct link to Add a lifecycle policy in GCP")
...
### Add a lifecycle policy in Azure[​](#add-a-lifecycle-policy-in-azure "Direct link to Add a lifecycle policy in Azure")
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/concepts/storage-mappings.md)
