---
title: "Imports"
description: "When you work on a draft Data Flow , your Flow specifications may be spread across multiple files. For example, you may have multiple  that read from collections defined in separate files, or you could store a  separately from its . You might also reference specifications that aren't in your local draft. For example, you might create a derivation with a source collection that is not in your local draft."
---

When you publish your draft, Flow automatically resolves references to specifications across the entirety of the [catalog](/concepts/catalogs/). This is possible because every entity in Flow has a globally unique name.
...
```
# Suppose we're in file "/path/dir/flow.yaml"import:  - sub/directory/flow.yaml        # Resolves to "file:///path/dir/sub/directory/flow.yaml".  - ../sibling/directory/flow.yaml # Resolves to "file:///path/sibling/directory/flow.yaml".  - https://example/path/flow.yaml # Uses the absolute url.
```

The import rule is flexible; a collection doesn’t have to do anything special to be imported by another, and [`flowctl`](/concepts/flowctl/) can even directly build remote sources:

```
# Test an example from a GitHub repository.$ flowctl draft test --source https://raw.githubusercontent.com/estuary/flow-template/main/word-counts.flow.yaml
```

## Fetch behavior[​](#fetch-behavior "Direct link to Fetch behavior")
...
## Import types[​](#import-types "Direct link to Import types")
...
```
import: - path/to/source/catalog.flow.yaml
```

A long-form variant also accepts a content type of the imported resource:

```
import: - url: path/to/source/catalog.flow.yaml   contentType: CATALOG
```
...
## JSON Schema `$ref`[​](#json-schema-ref "Direct link to json-schema-ref")
...
## Importing derivation resources[​](#importing-derivation-resources "Direct link to Importing derivation resources")

In many cases, [derivations](/concepts/derivations/) in your catalog will need to import resources. Usually, these are TypeScript modules that define the lambda functions of a transformation, and, in certain cases, the NPM dependencies of that TypeScript module.

These imports are specified in the derivation specification, *not* in the `import` section of the specification file.

For more information, see [Derivation specification](/concepts/derivations/#specification) and [creating TypeScript modules](/concepts/derivations/#modules).
...
[Edit this page](https://github.com/estuary/flow/edit/master/site/docs/concepts/import.md)
