---
title: "Captures"
description: "A  is how Flow ingests data from an external source. Every Data Flow starts with a capture."
---
...
Captures run continuously: as soon as new documents are made available at the endpoint resources, Flow validates their schema and adds them to the appropriate collection. Captures can process [documents](/concepts/collections/#documents) up to 16 MB in size.

![Flow capture diagram](/images/concepts/captures/assets/images/captures-new-4479ed989de046cd79db471d502bfc89.svg)

You define and configure captures in **Flow specifications**.

[See the guide to create a capture](/guides/create-dataflow/#create-a-capture)

## Connectors[​](#connectors "Direct link to Connectors")

Captures extract data from an endpoint using a [connector](/concepts/#connectors). Estuary builds and maintains many real-time connectors for various technology systems, such as database change data capture (CDC) connectors.

See the [source connector reference documentation](/reference/Connectors/capture-connectors/).

### Batch sources[​](#batch-sources "Direct link to Batch sources")

Flow supports running both first and third party connectors to batch sources as well as natively-written Estuary connectors. These connectors tend to focus on SaaS APIs, and do not offer real-time streaming integrations. Flow runs the connector at regular intervals to capture updated documents.

Third-party source connectors are independently reviewed and sometimes updated for compatibility with Flow. Estuary's [source connectors](/reference/Connectors/capture-connectors/) documentation includes all actively supported connectors. If you see a connector you'd like to prioritize for access in the Flow web app, [contact us](mailto:support@estuary.dev).
...
### Discovery through `flowctl`[​](#discovery-through-flowctl "Direct link to discovery-through-flowctl")
...
Where the provided `flow.yaml` file includes your capture [specification](#specification). If your chosen source `flow.yaml` includes more than one capture specification, you will need to specify one using the command's `--capture` parameter.
...
## Automatically update captures[​](#automatically-update-captures "Direct link to Automatically update captures")
...
* The `evolveIncompatibleCollections` option determines how to respond when the discovered updates would cause a breaking change to the collection. If `true`, it will trigger an [evolution](/concepts/advanced/evolutions/) of the incompatible collection(s) to prevent failures.

In the Flow web app, you can set these properties when you create or edit a capture.

![Capture auto-discovery in the UI](/images/concepts/captures/assets/images/captures-auto-discover-ui-e21d902c34932277099c045d7d62ac99.png)
